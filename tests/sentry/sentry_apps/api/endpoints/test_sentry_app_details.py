...
class UpdateSentryAppDetailsTest(SentryAppDetailsTest):
    ...
    @with_feature({"organizations:sentry-apps-granular-events": False})
    @override_options({"staff.ga-rollout": True})
    def test_cannot_add_granular_events_without_flag(self) -> None:
        app = self.create_sentry_app(name="SampleApp", organization=self.organization)
        self.get_error_response(app.slug, events=["issue.resolved"], status_code=403)
